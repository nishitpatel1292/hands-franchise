...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full z-50 backdrop-blur-sm transition-colors duration-300 ${isScrolled || isMenuOpen ? 'bg-white' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center">
            <Building2 className="h-8 w-8 text-blue-600" />
            <span className={`ml-2 text-xl font-bold hover:text-blue-600 ${isScrolled ? 'text-black ' : isMenuOpen ? 'text-black' : 'text-white'}`}>H&S Franchise</span>
          </div>
          ...
          <div className="md:hidden">
            ...
            >
              {isMenuOpen ? <X className='h-6 w-6 text-black' /> : <Menu className={`h-6 w-6 ${isScrolled ? 'text-black' : 'text-white'}`} />}
            </button>
          </div>
        </div>
      </div>
      ...
      {isMenuOpen && (
        <div
          className={`md:hidden transition-all duration-500 ease-in-out transform ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full'} bg-white`}
        >
